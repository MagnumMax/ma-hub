...
1. **Не** хранить в проекте отдельный «наш список skills» (копия манифеста, дубль таблицы из `external-skills.md` и т.п.). Проект работает по latest хаба (или pin в `AGENTS.md`).
...
Проекты держат канон-файлы (`README.md`, `AGENTS.md`, `DESIGN.md`, `llms.txt` — эталон `templates/project-canonical-pack.md`) и команды с машины. Они **не копируют** `commands/` и `standards/` целиком.

При `/MA-revise-project`: если в проекте уже есть файлы с копиями стандартов / описаний MA / отдельный UI-чеклист — **не плодить ещё один набор**. Нужное и уникальное перенести в `AGENTS.md` / `DESIGN.md`; живые ссылки на старые имена **заменить** (поиск по репо), затем дубли **удалить** после «ок» (указатель на старом пути — только если этот путь ещё требуют скрипт/CI). Старый `docs/MA-STANDARDS.md` — мигрировать в `AGENTS.md` и удалить. `UI_UX_GUIDELINES.md` и `docs/design-system.md` — в `DESIGN.md` в том же пакете, не следующей задачей. Чисто продуктовые правила (бренд, домен) не сносить. Автоблоки в `AGENTS.md` (`nextjs-agent-rules`) не резать. `llms.txt`: не класть `public/`, если уже есть роут.
