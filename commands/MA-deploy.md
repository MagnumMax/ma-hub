...
**Этот `/MA-deploy` — единственный оркестратор релиза.** Не подменяй чужим ship/land-flow. Не подменяй глубокой чисткой `/MA-product-pass` (там simplify «убери ИИ-стиль»; здесь одно ревью волны + ponytail). Карта: `$MA_HUB_ROOT/templates/ma-command-map.md`.
...
Следуй `$MA_HUB_ROOT/templates/project-canonical-pack.md` (разделы «Ворота /MA-*» и «`/MA-deploy`: нет обязательных путей»).

Перед релизом сверься с `$MA_HUB_ROOT/standards/05-release-and-quality.md` и эталоном `$MA_HUB_ROOT/templates/release-must-work-paths.md`. Pin и Local deviations — в `AGENTS.md`. Заглушки в шаблоне **не** считаются списком путей.

`check-only` / `skip-merge` — тот же стоп без пакета / без путей.
...
| **auto** | `auto` (+ можно `check-only` / `skip-merge`) | Чинить блокеры без паузы (CI→эталон thin+дешёвый, хуки→шаблон, Critical, fails, ревью волны). Ponytail в 1A — с автофиксом |

**Safe:** толстый/дорогой CI / `pnpm test` в Actions / тяжёлые хуки / fail проверок → стоп, не commit, не push. Грязное дерево в начале — **не** блокер. В 1A Critical/High и явный ponytail чинятся сразу; оставшийся ponytail `net ≤ -80` после автофикса → стоп. Medium/Low → сводный backlog, пауза на решения.
...
```
0 → 1 (0W волна → 1A review волны → 1B проверки → 1C) → 2 (react-doctor)
...
**Анти-повторы (закон):** один полный test suite в Phase 1 (только локально) + один CI после push + один Vercel prod. Полный test на каждый commit 3.9, в git-хуках и в GitHub Actions — **запрещён**. Конфликты снимать в 3.95 **до** PR. Перед build — место на диске / очистка `.next`. `--no-verify` не дефолт. Эталон хуков: `$MA_HUB_ROOT/templates/git-hooks-ma-deploy.md`. Эталон CI: `$MA_HUB_ROOT/templates/ci-ma-deploy.md` (минуты = job’ы × запуски на коммит).

**Где что:** тяжёлое локально (1–3.5), **tests только здесь**; CI = только `{typecheck, lint}` в **одном** job с одним install; `pnpm test` в Actions / любой другой CI job / дубль `pull_request` без форков = **блокер** (не «подождать и пойти»).

| Проверка | Phase | Где |
|----------|-------|-----|
| Ревью волны (code-review + ponytail один раз) | 0W–1A | эталон `templates/segmented-review-ma-deploy.md` |
| typecheck / lint / i18n / tests | 1B (tests **только** локально; typecheck/lint ещё в CI 5) | scripts из плана |
...
4. **Полный test** — один раз в Phase 1, **только локально**. Не в CI, не в git-хуках. Тяжёлые хуки / test в Actions / дорогой CI (много job’ов, дубль PR) = блокер Phase 0.
...
8. **Ревью волны всегда** (Phase 0W–1A): смесь независимых зон = **несколько выкатов** (стоп, ждать какую волну сейчас — даже в `auto`). На выбранной волне — **один** `/code-review` (+ bugbot/security по типу темы, тоже по одному). **Не** нарезать волну на куски. **Не** ревьюить весь грязный ком вместе с отложенными темами. Чужие волны **не** блокеры: спрятать на время 1B–3.9, не чинить «заодно».
9. **Обязательные пути:** без блока в `AGENTS.md` релиз не стартует. На **проде** — только проверка, без правок кода. Вход — канон из `templates/release-must-work-paths.md`, агент сам пишет `.env.local`. Нет ролей/пользователей → **предложить** создать + демо, не молча. Создал за прогон сверх демо — **подчистить до письма клиенту**. Провал пути или неубитый тестовый объект → стоп, без 7.5, без «горячего» фикса. **Не** вызывать `/MA-revise-project` изнутри выката. Пароли не печатать.
10. **Ponytail один раз на волну — с доказательством.** Прочитать skill ≠ прогнать. Обычное ревью ≠ ponytail. Нет **одной** строки таблицы 2b (`net` или `Lean already. Ship.`) на выбранную волну → **стоп до 1B**. Не гонять ponytail по кускам и не на весь грязный ком.
...
2. Прочитай `.github/workflows/` (сверка с `templates/ci-ma-deploy.md`), `package.json` scripts, ветки, `supabase/` → `HAS_SUPABASE`, prod URL из Vercel. Загрузи обязательные пути из `AGENTS.md` (ворота выше уже прошли).  
3. Таблица «План проекта»: проверка → phase → в CI? → job → команда/skill.  
   Только локально: ревью волны + ponytail (доказательство = одна строка таблицы 2b, не в CI), **tests**, i18n, doctor, build, budget, supabase. Lint — локально в 1B **и** в CI.  
   На проде (Phase 7): обязательные пути из `AGENTS.md`.  
   `CI_JOBS_TO_WAIT` = тонкий job (эталон: один `typecheck-and-lint`).
4. **Thin + cheap CI gate** (эталон `$MA_HUB_ROOT/templates/ci-ma-deploy.md` / `ci-ma-deploy.yml`):
   - **Состав:** только typecheck и/или lint → ок. `pnpm test` / любой другой job (build, e2e, doctor, i18n…) → блокер «CI толще шаблона».
   - **Форма:** typecheck+lint в **одном** job с одним install. Два+ параллельных job’а с повторным install → блокер «CI дороже эталона».
   ...
   Закрытие: привести workflow к эталону (убрать test из Actions, оставить typecheck+lint). **safe** — стоп; **auto** — правь в дереве без commit (в 3.9). После смены имени job (`typecheck-and-test` → `typecheck-and-lint`) — напомнить обновить required checks в GitHub. Не «подождать толстые/дорогие job’ы».
5. Ветки `dev`/`main`; нет `dev` — создай и запушь.
6. **Hooks gate** (как CI gate): эталон `templates/git-hooks-ma-deploy.md`. Тяжёлый pre-commit / pre-push с полным `pnpm test` → блокер.  
   **safe** — стоп; **auto** — привести к шаблону в дереве. Не обходить `--no-verify`. Pre-push с typecheck один раз — ок; test в хуках — нет.
7. Не упаковывай commits в Phase 0.

**Gate:** план готов + CI thin+cheap + hooks light (или auto уже поправил) → Phase 1 (сначала 0W).

## Phase 1 — Локальные проверки (волны)

Смотрим вместе, чиним по очереди. Команды — из плана. До Phase 2 — закрыть Critical/fails этой волны.  
Эталон: `$MA_HUB_ROOT/templates/segmented-review-ma-deploy.md`.

| Шаг | Что | Параллель |
|-----|-----|-----------|
| **0W** | Зоны продукта → одна волна этого выката или стоп «какую волну сейчас» | нет |
| **1A** | Один раз на волну: `/code-review` → bugbot/security по типу темы → Critical/High fix → ponytail **с автофиксом** + строка 2b → backlog | нет |
| **1A-end** | Сводный backlog + **gate: одна строка 2b** → иначе стоп, не 1B | нет |
| **1B** | typecheck ‖ lint ‖ i18n ‖ tests **на изолированной волне** | да |
| **1C** | сводка → таблицы | нет |
| Фиксы | 1B fails **этой волны**: safe=стоп; auto=по одному без commit. Чужие волны — не чинить | нет |

Diff scope: сначала весь грязный ком (0W), ревью / ponytail / 3.9 — **только выбранная волна**. Не ждать 3.9.

**0W:** разложить весь diff scope по зонам продукта (~строк = added+deleted). Две+ независимых темы → таблица волн → **стоп**, ждать какую волну сейчас (даже `auto`). Одна связная тема → одна волна, сразу 1A. Не нарезать тему на куски. Эталон: `$MA_HUB_ROOT/templates/segmented-review-ma-deploy.md` §0W.

**1A (один раз на выбранную волну):**
1. Ревью **волны целиком** (эталон `templates/segmented-review-ma-deploy.md` A1). Не куски. Не весь грязный ком с отложенными темами. Не спрашивать «какое ревью».
   | Что | Когда |
   |-----|--------|
   | `/code-review` | **всегда** (костяк; в шаге 2 критичное чинить — закон выката) |
   | `review-bugbot` | волна меняет **поведение** (логика / поток / API / данные) — не только тексты |
   | `review-security` | волна про **вход, роли, платежи, персональные данные, секреты** |
   Не вызывать `/review` (меню «одно из двух» + пауза). Не подменять `/code-review` специалистами. Каждый специалист — не больше одного прохода. Все три на копирайт — нет.
2. **Сразу** исправить Critical и High **этой волны** в дереве (**даже в safe**). Чужую волну не чинить «заодно».
3. **Отдельный** проход `ponytail-review` по **diff этой волны** (эталон `templates/segmented-review-ma-deploy.md` A3).  
   - Прочитать SKILL с диска (нет файла → блокер `install-external-skills.sh`). **Прочитать / поставить skill ≠ прогнать.**  
   - Findings показать **в чате** (`L…: <tag> …` или `Lean already. Ship.`).  
   - **Сразу применить** явные `delete`/`stdlib`/`native`/`yagni`/`shrink`. Спорное — в backlog, не молча.  
   - **Одна строка** в таблицу 2b: `net: -<N>` **или** `Lean already. Ship.` + сколько явных правок (0 только если lean / net 0).  
   Без этой строки — не 1B.  
   Исключение MA-deploy: skill «только отчёт» — здесь после отчёта агент **вносит** правки. Глубокая уборка продукта — `/MA-product-pass`, не этот шаг.
4. Medium/Low и спорное → backlog волны.

**1A-end:** показать таблицу backlog.  
- **Доказательство ponytail (gate до 1B):** одна строка таблицы 2b на волну. Дыра → **стоп**, Таблица 6 «нет доказательства ponytail», не 1B.  
- **safe:** пауза на «чини / не чинить / отложить» (пакетом ок). Пустой backlog или «всё отложить» → 1B **только если** есть строка 2b.  
- **auto:** дешёвые однозначные Medium — починить; остальное в отчёт → 1B **только если** есть строка 2b.

**Если дыру нашли позже** (уже 1B / упаковка, ещё не push): не выкладывать. Снова шаг 3 по **этой** волне → если код менялся, снова 1B → 3.8/3.9 заново, если уже упаковывали.

**Изоляция перед 1B:** ошибки в файлах **другой** волны — не блокер. Если suite красный из‑за них — спрятать их пути (`git stash push -u -- <пути>`) на время 1B–3.9, прогнать проверки на волне, упаковать, **вернуть** stash. Не чинить чужую тему, чтобы выложить эту.

**1B:** scripts из плана; Next typegen если нужен. Build **не** здесь. Полный suite — **один раз** после 1A на **изолированной** волне. **Не начинать 1B**, пока нет строки 2b.

**1C / loop:** safe — стоп на блокере 1B **этой волны**; auto — fix в дереве волны, макс. 5 итераций. До 3.9 запрещено коммитить «чтобы скорее» / спрашивать план commits / один большой commit всего diff / коммитить чужую волну.

Ориентир порядка в 3.9: CI → hooks → feat/fix → critical fixes → ponytail из 1A → typecheck/lint/i18n/tests → doctor+baseline → build/budget → supabase.
...
Когда 1–3.5 зелёные **и 3.8 закрыта** **и есть строка 2b** (ponytail на волну). Агент **сам**, без спроса. При `check-only` — в конце успеха (после 3.8).

```bash
export MA_ATOMIC_PACKING=1   # на всю серию commits
```

Одна причина = один commit. Только файлы **выбранной волны** (именованные пути / hunks). Файлы следующих волн **не** стейджить. После 3.9 **вернуть** спрятанные чужие волны (stash pop), чтобы они снова лежали грязными. Не полный test на каждый commit. Не `--no-verify` как обычный путь. Не `rebase -i` / `add -i`. Уже запушенное не переписывать. Грязное после «успеха» без 3.9 = ошибка — **кроме** явно отложенной следующей волны.
...
Skip при `check-only` / `skip-merge`. Skills: `vercel-cli`, `deployments-cicd`; путь — **Vercel MCP**. Эталон путей: `$MA_HUB_ROOT/templates/release-must-work-paths.md`. Ключи входа: `$MA_HUB_ROOT/templates/env-qa-test-accounts.snippet`.
...
3. Набор путей:
   - все строки `always`;
   - плюс `if-touched`, чья **Область** совпадает с выбранной волной;
   - весь каталог — только по явной просьбе или если релиз крупный (новый портал / много зон).
   Показать таблицу «сегодня гоняем» и сразу идти (без «ок?», если выбор однозначный).
4. Входы: канон из `$MA_HUB_ROOT/templates/release-must-work-paths.md`. Нет ключей в `.env.local` или другие значения → **сам дописать** канон (не спрашивать пароль). `.env.example` — email канона, пароль пустой. Не печатать пароли.
5. Войти на **prod**. Нет пользователя / нет роли / вход не пускает → предложить создать менеджера и клиента с канон-входом и наполнить демо-данными. **Даже в `auto` — ждать «ок».** Молча не создавать. После «ок» — создать, проверить вход.
6. Smoke B: `agent-browser` на живом **prod** URL — пройти **каждый выбранный путь**. Только проверка: **не** чинить исходники, **не** commits, **не** полный `/qa`. Нет браузера → ⚠️, не фейковый ✅. Путь молча не пропускать. Пароли — `[REDACTED]`.
7. Уборка на проде: всё, что этот прогон создал **сверх** базового QA-демо, удалить до 7.5. Учётки и их демо не трогать. Не смог подчистить → стоп, в отчёте что осталось.
8. Провал выбранного `always` или выбранного `if-touched` (даже в `auto`): **стоп** → не Phase 7.5 → не чинить на выложенном проде → не откатывать прод из чата. Чинить следующим кругом `/MA-deploy`. Косметика вне набора — в отчёт, не блокер.

**Gate:** READY + A OK + все выбранные пути OK + уборка OK (или явный fallback «нет браузера») = успех → **Phase 7.5**. Провал пути, отказ создать учётки, или неубитый тестовый объект = не успех, 7.5 запрещён.
...
| 1 | Сводка простым языком: этап, safe/auto, блокеры?, волна (зона сейчас / что отложено), commits, PR, merge, Vercel, обязательные пути, ponytail (строка 2b?), Telegram клиенту, уборка мусора |
| 2 | Шаги 1–3.5 / 3.8 / 3.9 / 3.95 — результат |
| 2a | Волна: зона / ~строк / Область / специалисты (— / bugbot / security / оба); Critical/High закрыты? чужие волны спрятаны и возвращены? |
| 2b | **Обязательно. Одна строка.** Ponytail волны: `net` или `Lean already. Ship.` + сколько явных правок + backlog. Пусто = блокер. |
...
| 5a | Обязательные пути; роли; `.env.local` канон записан?; учётки на проде есть / ждали ок создать+демо; что создали сверх демо и убрали |
...
**Этап работы** (одна фраза): жду обязательные пути / жду какую волну выкатывать / review волны / ponytail волны / нет доказательства ponytail / жду решения по backlog / проверки без упаковки / жду решения / уборка мусора до commits / жду что удалить / упаковываю / main→dev / выкладываю / жду CI / жду Vercel / гоняю пути на проде / жду ок создать QA-учётки и демо / убираю тестовые данные с прода / жду решение по Telegram / жду ключи Telegram / согласование текста клиенту / уборка хвоста / готово на проде / только проверки.
...
- Dirty tree в 0–3.5 — **не** стоп; commits только в 3.9 (только выбранная волна; хвост следующих волн может остаться грязным)
- Успех без 3.9 при грязном дереве; push/PR без 3.95
- Смесь независимых зон в одном выкате без выбора волны; нарезка волны на куски для ревью; 3.9 коммитит файлы вне волны
- Critical/High волны не закрыты; нет доказательства ponytail волны (нет строки таблицы 2b с `net` или `Lean already. Ship.`); «прочитал skill» / обычное ревью ≠ ponytail; переход к 1B / 3.9 / push без 2b; ponytail по кускам или на весь грязный ком; ponytail `net ≤ -80` после автофикса в safe; нет `ponytail-review` на диске; ревью чужих волн «заодно»; чинить чужую волну, чтобы проверки стали зелёные
- Fail typecheck/lint/i18n/test; doctor errors/score; disk < 1 GiB / повторный ENOSPC
- Fail build/budget — **push запрещён**
- Supabase в diff без плана; красный CI; `pnpm test` в Actions; CI толще thin или дороже эталона (много job’ов / дубль PR); тяжёлые хуки / test в хуках
- Нет блока обязательных путей (и нет ответа «дописать здесь» / «сначала сверка»); запуск `/MA-revise-project` изнутри выката
- Провал выбранного обязательного пути на проде; отказ создать QA-учётки; неубитый тестовый объект после прогона; переход к 7.5 после провала пути или без уборки; правка кода на уже выложенном проде; пароль тестового входа в чат/отчёт/git (кроме канона в хабе)
...
| 0W | зоны продукта → одна волна или стоп «какую сейчас» → `templates/segmented-review-ma-deploy.md` §0W |
| 1A | `/code-review` **всегда** (волна целиком) → `review-bugbot` если поведение → `review-security` если вход/роли/платежи/PII/секреты (**не** `/review`, по одному) → Critical/High fix → **один** `ponytail-review` **с автофиксом** + строка 2b (иначе стоп до 1B) |
| 1A-end | сводный backlog → решения (safe) / дешёвые Medium (auto) |
| 1B | typecheck ‖ lint ‖ i18n ‖ tests на изолированной волне (чужие пути спрятаны) |
...
| 6 stuck | `babysit` |
| 7 | Vercel MCP + `vercel-cli` / `deployments-cicd`; канон в `.env.local` → нет ролей: предложить создать + демо → пути → `agent-browser` → уборка сверх демо |
