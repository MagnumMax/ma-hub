...
### 7.5a — Проверка ключей (обязательная пауза, если нет)
...
3. **Если чего-то нет — не продолжать молча.** Простым языком сказать, **какие ключи добавить и куда** (чат/топик → `.env.local` продукта; бот → `~/.config/ma-hub/telegram.env`).  
   Ждать: **«готово»** (пользователь добавил) / **«пропустить уведомление»** (только этот релиз) / значения ключей в чат (агент допишет в `.env.local`, токен машины — только если пользователь явно дал).
4. После «готово» — **перечитать env и проверить снова**. Ещё пусто → снова сказать, что не хватает, не идти дальше по 7.5.  
   «Пропустить уведомление» → ⚠️ в отчёте, дальше Отчёт (прод уже успешен).

Даже в **`auto`**: отсутствие ключей = пауза на добавление (или явный пропуск), не тихий skip.

### 7.5b — Черновик и отправка

5. Ключи ок → собрать изменения релиза → черновик **деловым языком**: продукт + дата; до **10** пунктов «страница/функция — что»; без путей, SHA, жаргона.
6. Показать черновик. **Даже в `auto` — пауза:** «Отправить клиенту? **ок** / правки: … / **не слать**».
7. После «ок»: `"$MA_HUB_ROOT/bootstrap/telegram-customer-update-send.sh"` (stdin или `--file`). Не печатать токен. Ошибка API → сообщить, не откатывать прод.

**Gate:** ключи проверены + (отправлено / «не слать» / явный пропуск шага) → Отчёт.
...
| 5b | Phase 7.5: ключи ок? / черновик согласован? / отправлено / пропуск (явный) |
| 6 | Блокеры (только если есть) |

**Этап работы** (одна фраза): проверки без упаковки / жду решения / упаковываю / main→dev / выкладываю / жду CI / жду Vercel / жду ключи Telegram / согласование текста клиенту / готово на проде / только проверки.
...
| 7.5 | проверка ключей → (пауза если нет) → черновик → «ок» → `bootstrap/telegram-customer-update-send.sh` |
