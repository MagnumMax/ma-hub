...
5. Собрать изменения релиза → черновик по эталону `$MA_HUB_ROOT/templates/telegram-customer-update-ma-deploy.md`: **English**; HTML: `<b>#{N} Update, {date}</b>`; пункты `<b>{Zone} → {Sub-area}:</b> {fact}` с пустой строкой между ними; факт **очень короткий** (ориентир ≤12–15 слов, цель — в ~2 раза короче полной фразы, читается с первого взгляда). Перед показом пользователю укоротить ещё раз, если длинно. Если нечего сказать клиенту — «нет (только техника)».
