...
```
0 → 1 (0W волна → 1S сегменты → 1A review по кускам → 1B проверки → 1C) → 2 (react-doctor)
...
| Проверка | Phase | Где |
|----------|-------|-----|
| Сегментный review (code-review + ponytail) | 0W–1A | эталон `templates/segmented-review-ma-deploy.md` |
...
8. **Сегментный review всегда** (Phase 0W–1A): логические куски в одном дереве — **без** веток/PR на сегмент. Не подменять одним скопом на весь diff. Смесь независимых зон = **несколько выкатов** (стоп, ждать какую волну сейчас — даже в `auto`). Внутри волны кусков столько, сколько нужно; потолка «~8» нет. Мягкий ориентир ~400–800 строк на кусок — не жёсткий лимит.
...
| **1S** | План логических сегментов **внутри волны** (как split-to-prs, **без** веток/PR; без потолка «~8») | нет |
| **1A** | По каждому сегменту: `/code-review` всегда → bugbot/security по типу куска → Critical/High fix → ponytail **с автофиксом** + строка 2b → backlog | нет (сегменты строго по порядку) |
...
Diff scope: сначала весь грязный ком (0W), ревью и 3.9 — **только выбранная волна**. Не ждать 3.9.

**0W:** разложить весь diff scope по зонам продукта (~строк = added+deleted). Две+ независимых темы → таблица волн → **стоп**, ждать какую волну сейчас (даже `auto`). Одна связная тема → одна волна. Не сжимать смесь «в восемь кусков». Эталон: `$MA_HUB_ROOT/templates/segmented-review-ma-deploy.md` §0W.

**1S:** нарезать **только выбранную волну** на логические куски (смысл / владельцы / фундамент→потребители). Мягкий ориентир **~400–800 строк** на кусок: мелочь (~<200 той же темы) сливать; больше ~800 резать, если внутри независимые подтемы, иначе оставить связанным. Число сегментов — сколько нужно, **без** потолка «~8». В таблице: **~строк**, **Область** (для `if-touched`) и **ревью сверх `/code-review`** (bugbot / security / оба / —) по A1 — без «ок?» какое ревью. Маленький diff → **1 сегмент**, но полный цикл 1A всё равно. После выбранной волны показать таблицу кусков и сразу продолжать (без «ок?» на план кусков). **Запрещено** создавать ветки/PR на кусок.

**1A (на каждый сегмент):**
1. Ревью **этого куска** (эталон `templates/segmented-review-ma-deploy.md` A1). Не весь diff скопом. Не спрашивать «какое ревью».
   | Что | Когда |
   |-----|--------|
   | `/code-review` | **всегда** (костяк; в шаге 2 критичное чинить — закон выката) |
   | `review-bugbot` | кусок меняет **поведение** (логика / поток / API / данные) — не только тексты |
   | `review-security` | кусок про **вход, роли, платежи, персональные данные, секреты** |
   Не вызывать `/review` (меню «одно из двух» + пауза). Не подменять `/code-review` специалистами. Bugbot/Security — по файлам сегмента. Оба специалиста на одном куске ок; все три на копирайт — нет.
2. **Сразу** исправить Critical и High в дереве (**даже в safe** — исключение этой волны; из **всех** проходов шага 1).
...
Одна причина = один commit. Только файлы **выбранной волны** (именованные пути / hunks). Файлы следующих волн **не** стейджить. Не полный test на каждый commit. Не `--no-verify` как обычный путь. Не `rebase -i` / `add -i`. Уже запушенное не переписывать. Грязное после «успеха» без 3.9 = ошибка — **кроме** явно отложенной следующей волны.
...
| 1 | Сводка простым языком: этап, safe/auto, блокеры?, волна (зона сейчас / что отложено), сегменты, commits, PR, merge, Vercel, обязательные пути, ponytail (таблица 2b полная?), Telegram клиенту, уборка мусора |
...
- Смесь независимых зон в одном выкате без выбора волны; сжимать смесь в «~8 сегментов»; 3.9 коммитит файлы вне волны
...
| 1S | нарезка **волны** как `split-to-prs` (план; без веток/PR; ~400–800 строк мягко; без потолка «~8») → тот же эталон |
| 1A | `/code-review` **всегда** → `review-bugbot` если поведение → `review-security` если вход/роли/платежи/PII/секреты (**не** `/review`) → Critical/High fix → **отдельный** `ponytail-review` **с автофиксом** + строка таблицы 2b (иначе стоп до 1B) |
