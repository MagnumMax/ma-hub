...
Если релиз — большая фича без недавнего `/MA-product-pass` и в backlog/ponytail ещё много канцелярита — **не блокер выката**, но в итоге одна строка: «код ещё шумный → после прода или до следующего крупного куска: `/MA-product-pass`». После **готово** — «Следующие задачи» (4 пункта), не на паузе CI/Vercel/Telegram.
