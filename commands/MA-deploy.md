...
### 7.5-0 — Сначала: слать клиенту или нет

**Даже в `auto` — пауза.** Часть релизов чисто технические (CI, зависимости, внутренний рефакторинг) — клиенту писать не нужно.

Спросить простым языком:

> Уведомить клиента в Telegram об этом релизе?  
> **да** / **нет (только техника)**

- **нет (только техника)** → ⚠️ в отчёте «Telegram: пропуск, технический релиз», дальше Отчёт. **Не** требовать ключи, **не** писать черновик.
- **да** → 7.5a → 7.5b.

Тихий skip без этого вопроса **запрещён**.
...
5. Собрать изменения релиза → черновик по эталону `$MA_HUB_ROOT/templates/telegram-customer-update-ma-deploy.md`: заголовок `{Продукт} — обновление, {дата словами}`; строки вида `{Зона}: {что изменилось}` (без нумерации и без «экран — описание»); до 10 строк; без путей, SHA, жаргона. Если по сути нечего сказать клиенту — предложить сменить решение на «нет (только техника)» или одну нейтральную строку на ваше усмотрение.
6. Показать черновик. **Пауза:** «Текст для клиента ок? **отправить** / правки: … / **не слать**».
7. После «отправить»: `"$MA_HUB_ROOT/bootstrap/telegram-customer-update-send.sh"` (stdin или `--file`). Не печатать токен. Ошибка API → сообщить, не откатывать прод.

**Gate:** ответ на 7.5-0 + (пропуск | отправлено | «не слать» после черновика) → Отчёт.
...
| 5b | Phase 7.5: слать клиенту? / ключи? / текст согласован? / отправлено / пропуск |
...
| 7.5 | «слать клиенту?» → (если да) ключи → черновик → «отправить» → `bootstrap/telegram-customer-update-send.sh` |
