...
**1S:** нарезать diff на логические куски (смысл / владельцы / фундамент→потребители). В таблице сегментов указать **Область** (для отбора `if-touched` путей) и **ревью сверх `/code-review`** (bugbot / security / оба / —) по правилу A1 — без «ок?» какое ревью. Маленький diff → **1 сегмент**, но полный цикл 1A всё равно. Показать таблицу сегментов и сразу продолжать (без «ок?» на план). **Запрещено** создавать ветки/PR на кусок.
