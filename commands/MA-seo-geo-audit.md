...
Карта ритуалов: `$MA_HUB_ROOT/templates/ma-command-map.md`. Аудит и правки SEO **не** выкат.
...
Следуй `$MA_HUB_ROOT/templates/project-canonical-pack.md` (раздел «Ворота /MA-*»). Нет тройки README/AGENTS/DESIGN — предложи создать; аудит **можно** после «ок»/«позже» (иначе карту для ИИ не из чего честно собрать). **Нет `llms.txt` — не стоп:** этот ритуал его и пишет (или обновляет уже существующий генератор/роут). Не класть `public/llms.txt`, если `/llms.txt` уже отдаёт код.

Учти pin и Local deviations из `AGENTS.md`. Локальные SEO/GEO-чеклисты проекта (если есть) учитывай, но **не подменяй** ими треки Aaron.

**`llms.txt` — обязательный выход** режимов с GEO (`full` / `page` / `quick` / `pre-deploy` / `post-deploy`): собрать по `$MA_HUB_ROOT/skills/seo-geo-audit/references/llms-txt.md` (после трека `geo-content-optimizer`, не вместо него). Сначала найти роут/генератор в репо. Показать черновик в отчёте. На диск **или в генератор** — после «ок»/«чини», пункт «обновить llms.txt» по умолчанию **да**. Режимы `tech` / `structure` / `authority` — файл не переписывать, если не попросили. Не выдумывать URL. Localhost не выдавать за публичную цитату.
...
**Политика:** смотрим параллельно до **одного** итогового отчёта; не паузь между волнами ради «промежуточного сохранения». Правки сайта — только после «чини» / выбора пунктов, и **по очереди** (сначала доведи выбранное, потом один итог).  
...
- Файлы на диск — только если я явно попросил сохранить. **Исключение:** `llms.txt` (черновик в отчёте, запись после «ок»/«чини», дефолт да; если в репо уже есть роут/генератор — править **его**, не плодить `public/llms.txt`) — эталон `skills/seo-geo-audit/references/llms-txt.md`. Отчёт аудита в `memory/` — по-прежнему только по просьбе.

## Выход (всегда)

**Сначала** блок «Статус процесса» из `$MA_HUB_ROOT/standards/00-operating-model.md`:

| Поле | Значение |
|------|----------|
| Сейчас | идёт процесс / нужно ваше решение / готово |
| Что происходит | … |
| Ждём | ничего / ваш ответ про среду·URL / «чини» · выбрать пункты |
| Обычно занимает | — / волны аудита ~10–40 мин / пока не ответите |
| Что от вас | ничего / ответить про среду·URL / «чини» · выбрать пункты бэклога |

При стопе на Шаге 0: **Сейчас = нужно ваше решение**.  
После готового аудита до правок: **Сейчас = нужно ваше решение** (чинить или нет).  
Пока идут волны — не слать промежуточный «статус аварии»; один итог в конце.

Затем формат из `seo-geo-audit/references/report-template.md`:
...
9. **Черновик `llms.txt`** (если режим с GEO) — полный текст или diff; спросить «обновить живой источник (файл **или** роут)? **да** (рекомендую) / нет»

**Стоп после отчёта.** Не начинай правки кода, пока я не скажу «чини» / не выберу пункты бэклога.  
«чини» без оговорки по карте ИИ → **сначала записать живой `llms.txt`** (файл или генератор), потом выбранные пункты бэклога.  
Правки SEO **не** равны выкату: после выбранных P0, если нужен прод — `/MA-deploy`. Повтор аудита живого сайта: `/MA-seo-geo-audit <url> quick prod`.

После **готового** отчёта (или после согласованных правок) — блок «Следующие задачи» (4 пункта). На стопе Шага 0 (среда) — не ставить этот блок, только статус и вопрос.
