...
1. Phase 7 прошла: production READY + smoke OK (или явный smoke-fallback).
2. Агент готовит **черновик** сообщения.
3. **Пауза — бизнес-решение:** показать черновик пользователю → ждать **«ок» / правки / «не слать»**.
4. Только после «ок» — отправка скриптом ниже.
5. Нет `COMPANY_TELEGRAM_CHAT_ID` или токена → в отчёте ⚠️, релиз всё равно считается успешным (уведомление — дополнение, не блокер прода).

Даже в режиме **`auto`** отправка клиенту **не** делается молча — всегда согласование текста.
