...
| Сразу под заголовком — одна строка сути (2–4 темы) | Пропускать суть; эссе; копировать все буллеты в одну фразу |
...
Vehicle roles, customer statements, and branded PDFs.
...
- **Сразу под заголовком** (следующая строка, **без** пустой строки между ними) — **строка сути**: 2–4 темы релиза одним коротким предложением или перечислением. **Не** жирная. Ориентир **≤ ~12 слов**. Это «о чём апдейт», не список всех буллетов.
  - Хорошо: `Menu sync, live Pulse, and Roasters Club.` / `Invoice photos to Foodics (UAE) and Sheets (Hong Kong).`
  - Плохо: пропуск строки; повтор всех пунктов; «Various improvements»; язык коммитов.
...
- Пустая строка: **после строки сути**; **между секциями**. Не между заголовком и сутью; не между каждым `•`.
...
Антипример: вопрос «уведомить?» без текста; заголовок без строки сути; «топ-6» при богатом релизе; `Vehicles → Status: …`; пустая строка после каждого буллета; эссе Notion; список всех chore-коммитов.
...
4. После полного списка — **строка сути** под заголовком: 2–4 главные темы (не все буллеты). Затем можно слегка сжать **формулировки** буллетов (не выкидывать целые фичи). Если после отсева техники осталось мало — ок; если много — оставлять много.
